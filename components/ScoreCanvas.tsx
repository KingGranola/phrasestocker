...
interface ScoreCanvasProps {
    ...
    onChordClick: (chordId: string, x: number, y: number) => void;
...
const ScoreCanvas: React.FC<ScoreCanvasProps> = ({
    ...
    useEffect(() => {
        ...
        phrase.measures.forEach((measure, measureIndex) => {
            ...
            measure.chords.forEach(chord => {
                ...
                context.save(); context.font = "bold 20px Arial"; context.fillStyle = primaryColor; context.fillText(chord.symbol, chordX + 5, chordY); context.restore();
                const textWidth = context.measureText(chord.symbol).width;
                newChordBoxes.push({ x: chordX - 2, y: chordY - 25, w: textWidth + 14, h: 35, chordId: chord.id });
            });
            xPos += measureWidth;
        });
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        if (hitChord) { onChordClick(hitChord.chordId, e.clientX, e.clientY); e.stopPropagation(); return; }
